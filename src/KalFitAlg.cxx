...
StatusCode KalFitAlg::execute()
{
	...
	*/
	...
	for(int kj = 1; iter_trk != newtrkCol->end(); iter_trk++,kj++) {
		...
		rectrk->add = trkadd;
		//if(status_temp<=0) {
		//	if(debug_ == 4) cout<<"status_temp "<<status_temp<<" <=0!"<<endl;
		//	continue;
		//}
  ...
  //cout<<__LINE__<<" filter_fwd_anal helix: "<<track.a()<<" pivot "<<track.pivot()<<endl;
	...
	for( int i=0 ; i < nhit; i++ ) {
		...
		if(m_fix2TurnTrackInnerWall){
		  ...
		  if(debug_ == 4) cout<<endl<<"for inner wall: layer "<<layerf<<" lastHitLayer "<<lastHitLayer<<" trackLength "<<trackLength<<" lastHitTrackLength "<<lastHitTrackLength<<endl;
		  if(b_secondTurn){
		    ...
		    HepPoint3D IP(0,0,0);
		    if(debug_ == 4) std::cout<<__FILE__<<"   "<<__LINE__<<" change pivot to IP , pivot:"<<IP<<std::endl;
		    if(fabs(track.a()[3])>10) std::cout<<__FILE__<<"   "<<__LINE__<<" after convert pivot to IP helix.dz  "<<track.a()[3]<<std::endl;
		    track.pivot(IP,b_secondTurn);
		    if(debug_ == 4) cout<<"fix innerwall twice helix:"<<track.a()<<", Ea:"<<track.Ea()<<endl;
		    if(fabs(track.a()[3])>10) std::cout<<__FILE__<<"   "<<__LINE__<<" after convert pivot to next layer 0 helix.dz  "<<track.a()[3]<<std::endl;
		    innerwall(track, l_mass, way*-1); 
		    if(debug_ == 4) cout<<"after fix innerwall twice helix:"<<track.a()<<", Ea:"<<track.Ea()<<endl;
		    //turnOverPi = true;

		    //if(dchi2cut_all>0) {
		    //  for(int layid = 0; layid<43; layid++) {
		    //    KalFitTrack::dchi2cutf_anal[layid] = dchi2cut_all*3;
		    //    KalFitTrack::dchi2cutf_calib[layid] = dchi2cut_all*3;
		    //  }
		    //}
		  }
...
void KalFitAlg::kalman_fitting_anal(void)
{
  ...
  for(int l = 0; l < ntrk; l++) {
    ...
    for (vector<MdcRec_wirhit*>::iterator ii = pt.end()-1; ii != pt.begin()-1; ii--) 
      //for (vector<MdcRec_wirhit*>::iterator ii = pt.begin(); ii != pt.end(); ii++) // 2013-06-28 wll for CDC hits order
    { 
      hit_asso++;
      //if(ii==pt.begin()) outmostLayer=(*ii)->geo->Lyr()->Id();
      //if(outmostLayer==(*ii)->geo->Lyr()->Id()) continue;
    ...
    RecMdcKalTrackCol* kalcol,RecMdcKalHelixSegCol *segcol) 
{
  ...
  if(!(i_front_<0)){
	...
	l_mass++, flg <<= 1) {
      ...
      std::cout<<__FILE__<<"   "<<__LINE__<<" mhyp_  "<<mhyp_<<" flg "<<flg<<std::endl;
      if (!(mhyp_ & flg))  continue;
      if (l_mass == lead_) continue;
